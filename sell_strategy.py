...
class MyStrategy(bt.Strategy):
    ...
    def open_order(self):
        ...
        # try to open short
        if self.trend.l.up2down > thres or self.trend.l.down > thres:
            if self.shorttrend.l.down > thres or self.shorttrend.l.up2down > thres:
                if self.longtrend.l.up < (1 - thres) and self.longtrend.l.down2up < (1-thres):
    ...
    def close_order(self):
        thres = 0.8
        if not self.isbuy:
            if self.trend.l.down2up > thres or self.trend.l.up > thres or self.trend.l.none > thres:
                if self.shorttrend.l.down < 0.5:

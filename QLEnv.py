...
class QLEnv(gym.Env):
    ...
    def __init__(self, period, path, show_trade=True):
        ...
        self.model2 = get_model(period*2)
    ...
    def step(self, action):
        ...
        self.current_tick += 1
        self.updateState()
        if (self.current_tick >= len(self.closingPrices) or self.portfolio < 5000):
            self.done = True
    ...
    def reset(self):
        ...
        self.current_tick = self.period*2
    ...
    def get_state(self, period, model):
        prices = self.closingPrices[self.current_tick-period:self.current_tick]

        x = np.array(prices)
        x =np.reshape(x, (1,period))
    ...
    def updateState(self):
        ...
        state1 = self.get_state(self.period, self.model)
        state2 = self.get_state(self.period*2, self.model2)
        self.state =  (self.position, state1, state2)
